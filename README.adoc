...
Shareable code style/linting config to apply https://github.com/standardrb/standard[standardrb] with a select few overrides, as described in the <<overrides,What's not Standard>> section below.
...
1. Matz did not say frozen string literals were never going to happen
2. There are performance benefits to freezing string literals by default
3. Mutable strings are a bug-cause liability in data-processing code



==== That's not what matz said

Matz said frozen string literals as default wasn't going to happen _in Ruby 3_.
...
In https://github.com/standardrb/standard/pull/181[the discussion of this cop for Standard], a number of experienceD and trusted Ruby community members attest to the performance benefits of freezing string literals by default.
...
[TIP]
====
`rubocop -a` will fix many line length offenses, however it is conservative in which lines it tries to correct.
